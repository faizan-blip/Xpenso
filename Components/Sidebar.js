...
const Sidebar = () => {
 ...
 console.log( Cookies.get('next-auth.csrf-token')); 
    return (  
        ...
        <Image onClick={()=> router.push(`/?accesstoken=${accesstoken}&accessemail=${accessemail}`)} src={logo} alt=".." width={70} height={70} style={{boxShadow: "inset 2px 2px 5px #babecc,inset -5px -5px 10px #ffffff73", width:"6em" , height:"6em" , borderRadius:"50%" , padding:"0.5em 0.5em"}} />     
        ...
        <Stack marginTop='2.8em' bgcolor='' boxShadow='inset 2px 2px 5px #babecc,inset -5px -5px 10px #ffffff73' width='100%' height='auto' flexDirection='column' alignItems='center' padding='1.5em 0.5em' borderRadius='12px'>
         ...
         <Button onClick={()=> router.push(`/?accesstoken=${accesstoken}&accessemail=${accessemail}`)} variant='contained' sx={{borderRadius:"7px" , background:"#407bff !important" ,fontWeight:"700" , marginTop:"0.7em"}}>Know More</Button>
        </Stack>
         {
             (tokencr || accesstoken) && (
